...
-----
  ...
  # With a CSV or Parquet file:
  python sgr_bayesian.py --input well.csv --depth DEPT --gr-col GR --casings 1000 2500 4000

  # With NPHI-RHOB crossplot and model-based validation on other logs:
  python sgr_bayesian.py --las well.las --casings 1000 2500 4000 \
      --validate-curves RHOB NPHI DT --nphi NPHI --rhob RHOB \
      --output-dir results/
...
# ============================================================================
# Multi-format file reading  (LAS · CSV · Parquet)
# ============================================================================

def _guess_depth_column(columns) -> str:
    """Return the most likely depth-column name from *columns*."""
...
def _coerce_numeric_df(df: pd.DataFrame) -> pd.DataFrame:
    """Cast every column to numeric where possible."""
    out = df.copy()
...
def read_log_data(
    ...
    """
    Read well-log data from LAS, CSV, or Parquet into a depth-indexed DataFrame.

    Parameters
    ----------
    path      : path to .las / .csv / .parquet file
    depth_col : explicit depth column name (auto-detected if None)
    curves    : subset of curves to keep (all kept if None)

    Returns
    -------
    pd.DataFrame indexed by DEPTH, numeric columns only.
    """
    path = Path(path)
    suffix = path.suffix.lower()

    if suffix in {".las", ".las2"}:
        return load_las(path, curves=curves)   # existing loader keeps depth as index

    if suffix == ".csv":
        raw = pd.read_csv(path)
        if depth_col is None:
            depth_col = _guess_depth_column(raw.columns)
        raw = _coerce_numeric_df(raw)
        df = raw.set_index(depth_col)
        df.index.name = "DEPTH"
        if curves:
    ...
    raise ValueError(
        f"Unsupported file type: '{suffix}'. Use .las, .csv, or .parquet."
    )
...
def robust_rescale_01(x: np.ndarray, qlow: float = 0.01, qhigh: float = 0.99) -> np.ndarray:
    """
    Rescale *x* to [0, 1] using robust quantile clipping.

    Using the 1st and 99th percentiles instead of the raw min/max makes the
    SGR resistant to GR spikes or baseline outliers that would otherwise
    compress the whole log into a narrow band.

    Parameters
    ----------
    x     : input array (may contain NaN)
    qlow  : lower quantile for clipping (default 0.01)
    qhigh : upper quantile for clipping (default 0.99)

    Returns
    -------
    np.ndarray clipped to [0, 1], NaN preserved.
    """
...
def compute_sgr(
...
) -> Tuple[np.ndarray, List[PosteriorSamples], np.ndarray, np.ndarray]:
    """
    ...
    After all casing points the working GR is rescaled to [0, 1] using
    quantile-robust scaling (1st–99th percentile) so that GR spikes or
    baseline outliers do not compress the whole log.
    ...
    gr_shifted     : shifted-but-unscaled GR (NaN in transition zones)
    transition_mask: boolean array, True where data were removed
    """
    if config is None:
        config = GibbsConfig()

    gr_work         = gr_raw.copy().astype(float)
    transition_mask = np.zeros(len(depth), dtype=bool)
    posteriors      = []
    ...
    for i, z_cas in enumerate(sorted(casing_depths)):
        ...
        transition_mask[mask_trans] = True

        # Shift all data below the transition zone
        gr_work[depth > z_dbot_mean] -= theta_mean

    # Rescale to [0, 1] using quantile-robust scaling so outliers / GR spikes
    # do not collapse the whole log into a narrow band (improvement over raw min-max).
    gr_shifted = gr_work.copy()
    sgr        = robust_rescale_01(gr_work)

    return sgr, posteriors, gr_shifted, transition_mask
...
def run_model_validation(
    ...
    """
    Run the same Bayesian piecewise-linear model on *other* wireline logs.

    A significant shift (95 % credible interval for θ that excludes zero)
    on logs such as RHOB, NPHI, or DT suggests the casing zone may reflect
    a genuine lithological change rather than a tool artefact, and the SGR
    correction should be applied with caution.

    Parameters
    ----------
    df            : DataFrame indexed by depth containing the log curves
    gr_column     : name of the primary GR curve (excluded from validation)
    casing_depths : list of casing-point depths
    curves        : curves to test (auto-selected if None: RHOB, NPHI, DT, CALI)
    config        : GibbsConfig (uses defaults if None)

    Returns
    -------
    pd.DataFrame with one row per (curve, casing_point) combination.
    """
    from dataclasses import asdict  # local import – only needed here

    if config is None:
        config = GibbsConfig()

    if curves is None:
        default_validation_curves = ["RHOB", "NPHI", "DT", "CALI", "CAL"]
        curves = [c for c in default_validation_curves
                  if c in df.columns and c != gr_column]
    else:
        curves = [c for c in curves if c in df.columns and c != gr_column]
    ...
    for curve in curves:
        ...
        for z_cas in sorted(casing_depths):
            z_top = z_cas - config.d_max
            z_bot = z_cas + config.d_max
            mask  = (depth >= z_top) & (depth <= z_bot) & np.isfinite(vals)
            if mask.sum() < 10:
                rows.append({"curve": curve, "zcas": z_cas,
                             "status": "skipped", "note": "Too few valid points."})
                continue
            try:
                above_vals = vals[mask & (depth < z_cas)]
                mu_p_local = float(np.mean(above_vals)) if len(above_vals) > 0 \
                             else float(np.nanmean(vals[mask]))
                local_cfg = GibbsConfig(**{**config.__dict__, "mu_p": mu_p_local})
                post = run_gibbs_sampler(
                    vals[mask], depth[mask], z_cas, local_cfg
                )
                lo, hi = float(np.percentile(post.theta, 2.5)), \
                         float(np.percentile(post.theta, 97.5))
                rows.append({
                    "curve":             curve,
                    "zcas":              z_cas,
                    "theta_mean":        post.theta_mean,
                    "theta_95ci_low":    lo,
                    "theta_95ci_high":   hi,
                    "theta_significant": bool(lo > 0.0 or hi < 0.0),
                    "status":            "ok",
                    "note":              "",
                })
...
def plot_nphi_rhob_crossplot(
    ...
    """
    NPHI-RHOB crossplot for lithology consistency checks around casing points.

    Suggested in Oughton et al. (2014) to confirm that the zone around each
    casing point is predominantly shale before trusting the SGR correction.

    Parameters
    ----------
    df         : depth-indexed DataFrame containing NPHI and RHOB curves
    nphi       : neutron porosity curve name
    rhob       : bulk density curve name
    output_png : path to save the figure (not saved if None)
    ...
    if len(sub):
        ax.scatter(sub[nphi].values, sub[rhob].values, s=6, alpha=0.4, c="steelblue")
    ax.set_xlabel(f"{nphi} (v/v)")
    ax.set_ylabel(f"{rhob} (g/cc)")
    ax.set_title("NPHI – RHOB crossplot (lithology check)")
...
def plot_full_log(
    ...
    """
    Three-panel overview: Original GR | Shifted GR | SGR.

    Parameters
    ----------
    df            : DataFrame returned by process_well (must contain
                    GR_ORIG, GR_SHIFTED, SGR columns alongside the depth index)
    gr_column     : name used as the x-axis label on the first panel
    casing_depths : depths to mark with horizontal lines
    output_png    : save path (not saved if None)

    Returns
    -------
    matplotlib Figure
    """
    required = {"GR_ORIG", "GR_SHIFTED", "SGR"}
    missing  = required - set(df.columns)
    if missing:
        raise ValueError(f"DataFrame is missing columns: {missing}")

    dep   = df.index.values
    orig  = df["GR_ORIG"].values
    shift = df["GR_SHIFTED"].values
    sgr   = df["SGR"].values

    fig, axes = plt.subplots(1, 3, figsize=(12, 14), sharey=True)
    fig.suptitle("Bayesian SGR workflow – Oughton et al. (2014)", fontsize=12)

    def _panel(ax, x, label, xlim=None):
        valid = np.isfinite(x)
        ax.plot(x[valid], dep[valid], "k-", lw=0.5)
        for z in casing_depths:
            ax.axhline(z, color="grey", lw=1, ls="--")
        ax.set_xlabel(label)
        ax.set_title(label)
        ax.invert_yaxis()
        ax.grid(True, alpha=0.3)
    ...
    _panel(axes[0], orig,  f"GR raw ({gr_column}) [API]")
    _panel(axes[1], shift, "GR shifted [API]")
    _panel(axes[2], sgr,   "SGR [0 – 1]", xlim=(0, 1))
    axes[2].axvline(0.5, color="grey", lw=0.5, ls=":")
...
def process_well(
    ...
    validate_curves: Optional[List[str]]  = None,
    nphi_column:     Optional[str]        = None,
    rhob_column:     Optional[str]        = None,
    depth_col:       Optional[str]        = None,
) -> Tuple[pd.DataFrame, List[PosteriorSamples]]:
    """
    End-to-end SGR workflow.

    1. Load data (LAS / CSV / Parquet, or synthetic demo).
    2. Validate lithological consistency at casing points (t-test).
    3. Run the Bayesian Gibbs sampler at each casing point.
    4. Compute and save SGR, GR_SHIFTED, and TRANSITION_MASK.
    5. Optionally run model-based validation on other logs.
    6. Generate diagnostic plots including an optional NPHI-RHOB crossplot.
    7. Write all outputs to *output_dir* and save run_config.json.

    Parameters
    ----------
    las_path        : path to LAS, CSV, or Parquet file (synthetic if None)
    gr_column       : name of GR curve in the input file
    casing_depths   : list of casing depths in metres
    config          : GibbsConfig
    output_csv      : CSV output filename (placed inside output_dir)
    output_dir      : directory for all output files (created if needed)
    validate_curves : other log names to test for casing effects via model
                      (auto-selected from RHOB/NPHI/DT/CALI if None)
    nphi_column     : neutron porosity curve for NPHI-RHOB crossplot
    rhob_column     : bulk density curve for NPHI-RHOB crossplot
    depth_col       : depth column name for CSV/Parquet (auto-detected if None)

    Returns
    -------
    df         : input DataFrame with SGR, GR_ORIG, GR_SHIFTED, TRANSITION_MASK columns
    ...
    # ── Output directory ──────────────────────────────────────────────────────
    out_dir = Path(output_dir) if output_dir else Path("sgr_output")
    out_dir.mkdir(parents=True, exist_ok=True)

    # ── Load ──────────────────────────────────────────────────────────────────
    if las_path is not None:
        df = read_log_data(las_path, depth_col=depth_col)
        if gr_column not in df.columns:
            raise ValueError(
                f"Column '{gr_column}' not in file. "
                f"Available: {list(df.columns)}"
            )
        if casing_depths is None:
            raise ValueError("casing_depths must be provided with a real data file.")
    else:
        print("No input file provided – running synthetic demonstration.\n")
    ...
    # ── Validate lithological consistency (quick t-test) ─────────────────────
    print("\n── Validating lithological consistency at casing points (t-test) ──")
    ...
    sgr, posteriors, gr_shifted, trans_mask = compute_sgr(
        gr_raw, depth, valid_casings, config
    )
    df["GR_ORIG"]          = gr_raw
    df["GR_SHIFTED"]       = gr_shifted
    df["TRANSITION_MASK"]  = trans_mask
    df["SGR"]              = sgr
    ...
    # ── Model-based validation on other logs ─────────────────────────────────
    candidate_val_curves = validate_curves if validate_curves is not None \
        else [c for c in ["RHOB", "NPHI", "DT", "CALI"] if c in df.columns]
    available_val_curves = [c for c in candidate_val_curves
                            if c in df.columns and c != gr_column]
    if available_val_curves:
        print("\n── Model-based validation on other logs ──")
        val_df = run_model_validation(
            df, gr_column, valid_casings,
            curves=available_val_curves, config=config
        )
        val_path = out_dir / "validation_other_logs.csv"
        val_df.to_csv(val_path, index=False)
        print(f"Saved: {val_path}")
        # Print any significant findings
        if "theta_significant" in val_df.columns:
            sig = val_df[val_df["theta_significant"] == True]
            if len(sig):
                print("\n  ⚠  Significant casing effects detected on other logs:")
                print(sig[["curve", "zcas", "theta_mean",
                            "theta_95ci_low", "theta_95ci_high"]].to_string(index=False))

    # ── Plots ─────────────────────────────────────────────────────────────────
    if posteriors:
        plot_all_casing_points(
            gr_raw, depth, valid_casings, posteriors, config,
            output_path=str(out_dir / "sgr_posteriors.png"),
        )
        plot_full_log(
            df, gr_column, valid_casings,
            output_png=str(out_dir / "sgr_overview.png"),
        )
        plt.show()

    # Optional NPHI-RHOB crossplot
    if nphi_column and rhob_column \
            and nphi_column in df.columns and rhob_column in df.columns:
        plot_nphi_rhob_crossplot(
            df, nphi_column, rhob_column,
            output_png=str(out_dir / "nphi_rhob_crossplot.png"),
        )
        ...
        json.dump(
            {
                "input":          str(las_path),
                "gr_column":      gr_column,
                "casing_depths":  casing_depths,
                "valid_casings":  valid_casings,
                **config.__dict__,
            },
            f, indent=2, default=str,
        )
    print(f"Run config saved to: {cfg_path}")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--input",    metavar="FILE",  default=None,
                   help="Input file: LAS (.las), CSV (.csv), or Parquet (.parquet/.pq). "
                        "Omit to run the synthetic demonstration.")
    p.add_argument("--las",      metavar="FILE",  default=None,
                   help="Alias for --input (LAS files). Ignored if --input is given.")
    p.add_argument("--depth",    metavar="COL",   default=None,
                   help="Depth column name for CSV/Parquet (auto-detected if omitted).")
    p.add_argument("--gr-col",  metavar="NAME",   default="GR",
                   help="GR curve name in the input file (default: GR)")
    ...
    p.add_argument("--output",   metavar="FILE",  default="sgr_output.csv",
                   help="Output CSV filename (placed inside --output-dir, default: sgr_output.csv)")
    p.add_argument("--output-dir", metavar="DIR", default="sgr_output",
                   help="Directory for all output files (default: sgr_output/)")
    # ── Validation ──────────────────────────────────────────────────────────
    p.add_argument("--validate-curves", metavar="NAME", nargs="*",
                   default=None,
                   help="Other log curves to test for casing effects using the "
                        "full Bayesian model (default: RHOB NPHI DT CALI if present)")
    p.add_argument("--nphi",     metavar="COL",   default=None,
                   help="Neutron-porosity curve for NPHI-RHOB crossplot")
    p.add_argument("--rhob",     metavar="COL",   default=None,
                   help="Bulk-density curve for NPHI-RHOB crossplot")
    return p
...
if __name__ == "__main__":
    ...
    input_file = args.input or args.las   # --input takes precedence; --las is the alias
    ...
    process_well(
        ...
        validate_curves = args.validate_curves,
        nphi_column     = args.nphi,
        rhob_column     = args.rhob,
